...
namespace Poly
{
	...
	enum class FResourceBindPoint : uint32
	{
		NONE   = 0,
		VERTEX = FLAG(1),
		INDEX  = FLAG(2),
		...
		UNIFORM         = FLAG(3),
		CONSTANT        = UNIFORM,
		UNIFORM_TEXEL   = FLAG(4),
		UNIFORM_DYNAMIC = FLAG(5),

		STORAGE          = FLAG(6),
		UNORDERED_ACCESS = STORAGE,
		STORAGE_TEXEL    = FLAG(7),
		STORAGE_DYNAMIC  = FLAG(8),
		STORAGE_IMAGE    = FLAG(9),

		INDIRECT = FLAG(10),

		COLOR_ATTACHMENT = FLAG(11),
		RENDER_TARGET    = COLOR_ATTACHMENT,

		DEPTH_STENCIL = FLAG(12),

		SHADER_READ   = FLAG(13),
		SAMPLER       = SHADER_READ,
		SAMPLED_IMAGE = FLAG(14),
	...
	inline EDescriptorType ConvertBindpointToDescriptorType(FResourceBindPoint bindpoint)
	{
		...
		if (BitsSet(FResourceBindPoint::STORAGE, bindpoint))
			return EDescriptorType::STORAGE_BUFFER;
		if (BitsSet(FResourceBindPoint::UNIFORM, bindpoint))
			return EDescriptorType::UNIFORM_BUFFER;
		if (BitsSet(FResourceBindPoint::INPUT_ATTACHMENT, bindpoint))
			return EDescriptorType::INPUT_ATTACHMENT;
