...
namespace Poly
{
	...
	public:
		 ...
		 * @param passID - ID of the pass to be added
		 * @return true if pass is added successfully
		 */
		bool AddPass(const Ref<Pass>& pPass, const PassID& passID);

		/**
		 * Removes a pass from the graph - be aware of loose edges after this has been done
		 * @param passID - ID of the pass to be removed
		 * @return true if pass is removed successfully
		 */
		bool Removepass(const PassID& passID);

		/**
		 * Add a pass resource - pass resource link to the graph.
		 *
		 ...
		 * 									then use the global namespace of $ instead of renderPassName
		 *
		 *
		 ...
		 */
		bool AddLink(const PassResID& src, const PassResID& dst);

		/**
		 * Add an external pass - pass resource link to the graph.
		 *
		 * - Data-dependency:		By linking two passes' resources to each other a data-dependency will be created.
		 * 							This enforces the graph to keep the data between those passes dependent on each other,
		 * 							removing possible overlap in data usage
		 ...
		 *
		 *
		 * @param src - src external resource name
		 * @param dst - dst pass resource name
		 * @return true if link is added successfully
		 */
		bool AddLink(const ResID& src, const PassResID& dst);

		/**
		 * Add a pass-pass link to the graph.
		 *
		 ...
		 *
		 * @param src - src pass name
		 * @param dst - dst pass name
		 * @return true if link is added successfully
		 */
		bool AddLink(const PassID& src, const PassID& dst);

		/**
		 * Remove an already existing pass resource - pass resource link, must follow same structure of names as AddLink
		 * @param src - src pass-resource name
		 * @param dst - dst pass-resource name
		 * @return true if link is removed successfully
		 */
		bool RemoveLink(const PassResID& src, const PassResID& dst);

		/**
		 * Remove an already existing external resource - pass resource link, must follow same structure of names as AddLink
		 * @param src - src external resource name
		 * @param dst - dst pass-resource name
		 * @return true if link is removed successfully
		 */
		bool RemoveLink(const ResID& src, const PassID& dst);

		/**
		 * Remove an already existing pass-pass link, must follow same structure of names as AddLink
		 * @param src - src pass name
		 * @param dst - dst pass name
		 * @return true if link is removed successfully
		 */
		bool RemoveLink(const PassID& src, const PassID& dst);
		 ...
		 */
		bool AddExternalResource(const ResID& resID, uint64 size, FBufferUsage bufferUsage, const void* data = nullptr, bool autoBindDescriptor = true);
		 ...
		 */
		bool MarkOutput(const PassResID& passResID);
		 ...
		 * @return true if output could be added successfully
		 */
		bool UnmarkOutput(const PassResID& passResID);
	...
	private:
		...
		struct OutputKeyHasher
		{
			...
			{
				return std::hash<uint32>()(key.NodeID) ^ (std::hash<ResID>()(key.ResourceID) >> 1);
			}
		};
