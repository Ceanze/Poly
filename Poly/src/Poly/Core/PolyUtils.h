...
#define PVK_CHECK(F, ...)             \
	if (F != VK_SUCCESS)              \
	{                                 \
		POLY_CORE_ERROR(__VA_ARGS__); \
		assert(F);                    \
	}

// Cleanup of vectors of PVK types
#define PVK_VEC_CLEANUP(V) \
	for (auto& e : V)      \
	{                      \
		e.Cleanup();       \
	}
...
#define PVK_CLEANUP(handle, func) \
	if (handle != VK_NULL_HANDLE) \
	{                             \
		func;                     \
	}

#define POLY_VALIDATE(exp, ...)           \
	{                                     \
		if (!(exp))                       \
		{                                 \
			POLY_CORE_FATAL(__VA_ARGS__); \
		}                                 \
		assert(exp);                      \
	}
...
#define FLAG_CHECK(Enum, trueCase) \
	if (EnumCheck(Enum))           \
	trueCase
...
#define ENABLE_BITMASK_OPERATORS(e)                                   \
	...
	{                                                                 \
		return static_cast<e>(ENUM_CAST(e, lhs) | ENUM_CAST(e, rhs)); \
	}                                                                 \
	inline e& operator|=(e& lhs, e rhs)                               \
	{                                                                 \
		lhs = lhs | rhs;                                              \
	...
	{                                                                 \
		return static_cast<e>(~ENUM_CAST(e, lhs));                    \
	}                                                                 \
	inline bool BitsSet(e val, e flag)                                \
	{                                                                 \
		return ENUM_CAST(e, val & flag) > ENUM_CAST(e, 0);            \
	}                                                                 \
	inline bool EnumCheck(e val)                                      \
	{                                                                 \
		return ENUM_CAST(e, val) > 0;                                 \
	}